...
class Cart {
  public products: { productId: number; price: number }[] = [];
    ...
    if (this.products.length === 0) {
      ...
      this.products.forEach((p) => {
        ...
        if (product) {
          console.log(
            chalk.green.bold.italic(
              `\tPRODUCT ID: ${p.productId}, NAME: ${product.name}, PRICE: ${product.price}`
            )
          );
        }
      });
      console.log(
        chalk.magentaBright.bold.italic(`\tTOTAL PRICE: ${this.totalPrice}\n`)
      );
    }
  }
}
...
setTimeout(() => {
  ...
  async function purchasing(cart: Cart) {
    Items.push(
      new Product(
        "LAPTOP",
        202,
    ...
    while (isContinue) {
      ...
      let getCategoryItem = Items.filter(
        (categories) => categories.category === items.items
      );
      ...
      if (askForPurchaseItem.ask === "PURCHASE ITEM") {
        ...
        const selectedProduct = getCategoryItem.find(
          ...
          if (buyItems.buy) {
            ...
            while (!validAmount) {
              ...
              if (money.amount === cart.totalPrice) {
                ...
                console.log(
                  chalk.yellow.bold.italic(
                    `\tPLEASE SHARE YOUR DETAILS SO WE CAN SEND YOUR SELECTED ITEMS ONLINE.\n`
                  )
                );
                ...
                console.log(
                  chalk.green.bold.italic(
                    `\tYOUR ORDER WILL BE DELIVERED TO YOU WITH IN 3 DAYS.\n`
                  )
                );
